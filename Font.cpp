...
{
    TTF_Init();
    font = TTF_OpenFont((std::string("Data")+PATH_SEPARATOR+"Fonts"+PATH_SEPARATOR+"Roboto-Light.ttf").c_str(),20);
}
...
void render_text(int posx, int posy, std::string s, Uint8 brightness, Uint8 alpha, bool m_aligned)
{
    int offset = 0;
    std::deque<std::string> splitt = split(s,'\n');
    for (auto st: splitt)
